...
本轮工程硬化后，系统把“用户想做什么”和“系统当前能不能执行”分开建模。这样可以避免把模糊导航、知识问答和真正错误混在一起。

| 状态 / 意图 | 语义 | 是否错误 |
| --- | --- | --- |
| `INFO_QUERY` | 解释车辆知识、功能含义或安全概念，例如 `AEB是什么`、`讲一下制动距离` | 否 |
| `NEEDS_CLARIFICATION` | 当前输入不足以唯一确定目的地，需要用户补充城市、商圈、门店或完整地址 | 否 |
| `UNKNOWN` | 系统无法识别可服务的业务意图 | 是，策略阻断 |
| `BLOCKED` | 安全策略拒绝执行，例如 `关闭AEB`、`加速到100km/h` | 是，安全阻断 |

目的地解析也从简单字符串处理升级为独立边界：

```text
LocalIntentAgent
  -> SlotExtractor 抽取 destination_query
  -> DestinationResolver 判断是否唯一、是否需要澄清、是否可以调用 provider
  -> NEEDS_CLARIFICATION 或 CloudTripPlanning
```

面试时可以这样讲：

> 我把模糊目的地和未知指令分开了。比如“导航去北京”不是系统错误，而是一个正常的澄清状态；“AEB是什么”也不是未知指令，而是信息查询。这样状态建模更接近真实产品。
